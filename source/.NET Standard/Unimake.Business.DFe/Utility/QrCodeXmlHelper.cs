    ...
    internal static class QrCodeXmlHelper
    {
        ...
        public static void MontarQrCodeNFCe(XmlDocument conteudoXml, Configuracao configuracoes)
        {
            ...
            foreach (XmlNode nodeNFe in nodeListNFe)
            {
                ...
                //GAT TECNOLOGIA - capturar qrCode/urlChave já existentes no XML antes de recriar o grupo suplementar
                var qrCodeOriginal = string.Empty;
                var urlQrCodeOriginal = string.Empty;
                var urlChaveOriginal = string.Empty;

                var matchQrCode = new Regex("(<qrCode>)(.*?)(</qrCode>)").Match(conteudoXml.InnerXml);
                var matchUrlChave = new Regex("(<urlChave>)(.*?)(</urlChave>)").Match(conteudoXml.InnerXml);

                if (matchQrCode.Success)
                {
                    qrCodeOriginal = matchQrCode.Groups[2].Value;
                }

                var matchCHashQRCode = new Regex("(.*?)([?]p=.*)").Match(qrCodeOriginal);
                if (matchCHashQRCode.Success)
                {
                    urlQrCodeOriginal = matchCHashQRCode.Groups[1].Value;
                }

                urlChaveOriginal = matchUrlChave.Success ? matchUrlChave.Groups[2].Value : urlQrCodeOriginal;

                //GAT TECNOLOGIA - sempre recriar o grupo suplementar, removendo o existente antes
                if (elementNFe.GetElementsByTagName("infNFeSupl").Count > 0)
                {
                    var infNFeSuplNodes = conteudoXml.GetElementsByTagName("infNFeSupl");

                    while (infNFeSuplNodes.Count > 0)
                    {
                        var nodeToRemove = infNFeSuplNodes[0];
                ...
                if (elementNFe.GetElementsByTagName("infNFeSupl").Count <= 0)
                {
                    ...
                    //GAT TECNOLOGIA - manter a urlChave/urlQrCode já existentes no XML, quando informadas
                    if (!string.IsNullOrEmpty(urlChaveOriginal))
                    {
                        urlChave = urlChaveOriginal;
                    }
                    if (!string.IsNullOrEmpty(urlQrCodeOriginal))
                    {
                        urlQrCode = urlQrCodeOriginal;
                    }
